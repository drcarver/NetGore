...
public partial class MonsterLists : IMonsterLists
{
    ...
    public bool IsNPC(MonsterInfoViewModel monsterInfo)
    {
        var npc = NPCList.FirstOrDefault(m => m.Name == monsterInfo.Name);
        if (npc == null)
        {
            return false;
        }
    ...
    /// <param name="monsterInfo">The monster info</param>
    /// <returns>True if the monster is a NPC</returns>
    public bool IsWere(MonsterInfoViewModel monsterInfo)
    {
        return monsterInfo.Name.ToLower().StartsWith("were");
    }
}
